...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with the correct base URL configured for your account before running requests and replace the placeholders with your actual data. Timestamps use the ISO 8601 pattern YYYY-MM-DDTHH:mm:ssZ in Coordinated Universal Time (UTC).
    </Note>
  </Tab>
  <Tab title="Response">
