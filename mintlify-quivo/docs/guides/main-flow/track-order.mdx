...
- **Order ID:** The unique integer `orderId` returned when you created the order. Use the [`GET /orders` endpoint](/api-reference/#tag/orders) in the API Reference to find it.
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address before running requests and replace the placeholders with your actual data. Timestamps use the ISO 8601 pattern YYYY-MM-DDTHH:mm:ssZ in Coordinated Universal Time (UTC).
    </Note>
  </Tab>
  <Tab title="Response">
