...
description: Configure and manage event subscriptions via the API
---
...
- **API Credentials:** Your API key and a valid session token. See the [Authentication guide](/essentials/authentication) to learn how to obtain a session token.
...
- Replace all placeholders (for example, `<YOUR_SELLER_ID>`, `<YOUR_WEBHOOK_URL>`) with your actual values. IDs should be integers; URLs must be valid HTTPS endpoints; all other placeholders should be strings.
