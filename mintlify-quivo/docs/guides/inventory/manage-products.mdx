...
This guide teaches you how to retrieve article details, update articles, manage product images, and search your catalog. Each article has a unique stock keeping unit, or SKU, that identifies it. SKU stands for stock keeping unit.
...
- **Existing Articles:** You should have at least one article created. See the Create Products guide at /docs/guides/main-flow/create-products if you haven't created articles yet.
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Use the `articleId` returned when you created the article.
    </Note>
  </Tab>
  <Tab title="Response">
    ...
    ```
    Timestamps use the ISO 8601 format YYYY-MM-DDTHH:mm:ssZ: YYYY is the four-digit year, MM is the month, DD is the day, HH:mm:ss is the time, and Z indicates Coordinated Universal Time. ISO stands for International Organization for Standardization.
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Use the `query` parameter to filter results, `sort` to order them, and `pageSize` to limit the number of results.
    </Note>
  </Tab>
  <Tab title="Response">
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Include all fields you want to keep, not just the ones you're updating. The request body should contain the complete article data.
    </Note>
  </Tab>
  <Tab title="Response">
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Specify the `mimeType` query parameter, for example `image/jpeg` or `image/png`.
    </Note>
  </Tab>
  <Tab title="Response">
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `<UPLOAD_URL>` with the upload web address from Step 1. Replace `<PATH_TO_IMAGE_FILE>` with the path to your image file. Set the `Content-Type` header to match the mime type you specified in Step 1.
    </Note>
  </Tab>
  <Tab title="Response">
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Use the image `id` from the article details.
    </Note>
  </Tab>
  <Tab title="Response">
...
<Tabs>
  <Tab title="Request">
    ...
    <Note>
      Replace `${BASE_URL}` with your environment web address. Include the article data you want to validate in the request body.
    </Note>
  </Tab>
  <Tab title="Response">
